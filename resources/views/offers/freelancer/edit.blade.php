...
@section('page_content')
    ...
    <div class="container">
        ...
        <h1 class=" margin-bottom-20">
            <small>
                Edit <strong>{{ $offer->title }}</strong>
            </small>
        </h1>

        <form action="{{ route('offers.freelancer.edit', $offer->id) }}" method="post" name="job_form" enctype="multipart/form-data">
            @csrf
                            ...
                            <div class="row">
                                <div class="col-xl-8">
                                    <div class="submit-field">
                                        <h5>Offer Title</h5>
                                        <input type="text" class="with-border tippy" placeholder="Offer Title" name="title" value="{{ $offer->title }}" required>
                                    </div>
                                </div>
                                            ...
                                            @foreach ($categories as $category)
                                                <optgroup label="{{ $category->title }}">
                                                    @foreach ($category->Offer_sub_categories as $sub_category)
                                                        <option value="{{ $sub_category->id }}" {{ $sub_category->id == $offer->sub_category_id ? 'selected' : '' }} data-baseamount="{{ $sub_category->base_amount }}">{{ $sub_category->title }}</option>
                                                    @endforeach
                                                </optgroup>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-xl-12">
                                    <div class="submit-field">
                                        ...
                                        <textarea cols="30" rows="5" name="description" class="with-border tippy"
                                            placeholder="Lorem ipsum dolor sit amet, consectetur adipisicing elit. Amet blanditiis nemo nobis placeat. Atque, doloribus esse eveniet fuga fugiat illum ipsa labore magni molestiae mollitia nemo obcaecati totam unde ut." required>{{ $offer->description }}</textarea>
                                    </div>
                                </div>
