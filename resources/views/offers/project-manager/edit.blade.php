                        ...
                        <div class="row">
                            <div class="col-xl-8">
                                <div class="submit-field">
                                    <h5>Offer Title</h5>
                                    <input type="text" class="with-border tippy" placeholder="Offer Title"
                                        name="title" value="{{ $offer->title }}" required>
                                </div>
                            </div>
                                        ...
                                        @foreach ($categories as $category)
                                            <optgroup label="{{ $category->title }}">
                                                @foreach ($category->Offer_sub_categories as $sub_category)
                                                    <option value="{{ $sub_category->id }}" {{ $sub_category->id == $offer->sub_category_id ? 'selected' : '' }} data-baseamount="{{ $sub_category->base_amount }}">
                                    ...
                                    <div class="keywords-container">
                                        ...
                                        <div class="skills keywords-list" id="skills-keywords">
                                            @foreach($offer->skills as $skill)
                                                <span class="keyword"><span class="keyword-remove"></span><span class="keyword-text">{{ $skill->title }}</span></span>
                                            @endforeach
                                        </div>
                                        {{-- @endif --}}
                                        <div class="clearfix"></div>
                            ...
                            <div class="col-xl-4">
                                <div class="submit-field">
                                    ...
                                    <select class="with-border tippy" name="designer_level"
                                        ...
                                        <option value="">-</option>
                                        <option value="0" {{ $offer->minimum_designer_level == '0' ? 'selected' : '' }}>Anybody can apply</option>
                                        <option value="3" {{ $offer->minimum_designer_level == '3' ? 'selected' : '' }}>Minimum of 3 stars</option>
                                        <option value="5" {{ $offer->minimum_designer_level == '5' ? 'selected' : '' }}>5 stars</option>
                                    </select>
                                    <div class="clearfix"></div>
                                </div>
                            </div>
                            ...
                            <div class="col-xl-4">
                                <div class="submit-field">
                                    ...
                                    <input type="number" class="budget budget-input" placeholder="15000" min="{{ $offer->budget }}" value="{{ $offer->budget }}" name="budget" readonly>
                                </div>
                            </div>
                            ...
                            <div class="col-xl-12">
                                <div class="submit-field">
                                    ...
                                    <textarea cols="30" rows="5" name="description" class="with-border tippy"
                                        ...
                                        required readonly>{{ $offer->description }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="col-xl-12 mb-4">
                                <form action="{{ route('offer.images') }}" method="POST" id="contest-images-form"
                                    ...
                                    @csrf
                                    <input type="hidden" name="offer_id" id="offer_id" value="{{ $offer->id }}" required />
                                </form>
                            </div>
                            <div class="clearfix"></div>
                            ...
                            <div class="col-xl-6 timeline-container">
                                <div class="submit-field">
                                    ...
                                    <select class="with-border tippy" name="timeline" data-placeholder="No of designers">
                                        <option value="">-</option>
                                        @for ($i = 1; $i <= 30; $i++)
                                            <option value="{{ $i }}" {{ $offer->timeline == $i ? 'selected' : '' }}>{{ $i }} day{{ $i == 1 ? '' : 's' }}</option>
                                        @endfor
                                    </select>
                                    <div class="clearfix"></div>
                                </div>
                            ...
                            <div class="col-xl-6 offer-type-container">
                                <div class="submit-field">
                                    <h5>Offer Type</h5>
                                    <select class="with-border tippy" name="offer_type" data-placeholder="No of designers" disabled>
                                        <option value="">-</option>
                                        <option value="public" {{ $offer->offer_user_id == null ? "selected" : '' }} >Public Offer</option>
                                        <option value="private" {{ $offer->offer_user_id != null ?  "selected" : '' }}>Private Offer</option>
                                    </select>
                                    <div class="clearfix"></div>
                                </div>
                            </div>
                            ...
                            <div class="col-xl-6 offer-user-container">
                                <div class="submit-field">
                                    <h5>Select User</h5>
                                    <select class="with-border tippy" name="offer_user" data-placeholder="No of designers" disabled>
                                        <option value="" data-user="">-</option>
                                        <option value="public" data-user="all">All Freelancers</option>
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}" data-user="freelancer" {{ $user->id == $offer->offer_user_id ? 'selected' : '' }}>
...
@section('page_scripts')
    ...
    <script>
        ...
        submit_offer_form_btn.on('click', () => {
            ...
            console.log(`{{ route('offers.project-manager.edit', $offer->id) }}`)

            var link = `{{ route('offers.project-manager.edit', $offer->id) }}`
            // return
            fetch(link, {
                    method: 'post',
                    headers: {
                ...
                .then(async responseJson => {
                    if (responseJson.success) {
                        ...
                        setTimeout(() => {
                            ...
                            // window.location =
                            //     `{{ url('offers/project-manager/validate-update/') }}/${responseJson.offer_id}/${responseJson.old_budget}`;
                            window.location =
                                `{{ url('offers/project-managers/') }}/${responseJson.offer_slug}`;
                        }, 2000)
                    } else {
                        Snackbar.show({
