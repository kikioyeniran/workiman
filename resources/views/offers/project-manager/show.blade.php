    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-xl-8 col-lg-8 content-right-offset">
                ...
                @if (auth()->check() && ((auth()->user()->id == $offer->user->id) || $offer->interests->where('assigned',
                true)->where('user_id', auth()->user()->id)->first()))
                ...
                <div class="margin-top-20">
                    ...
                    @forelse ($offer->comments as $comment)
                        <div
                            class="each-comment-container comment-{{ $comment->user_id == auth()->user()->id ? 'right' : 'left' }}">
                            {{-- <div> --}}
                            ...
                            @elseif ($comment->content_type == 'image')
                                <div class="comment-content files">
                                    @foreach (json_decode($comment->content) as $comment_file)
                                        <div>
                                            <img src="{{ asset("storage/offer-comment-images/{$comment->offer->id}/{$comment_file}") }}"
                                                alt="" class="img-thumbnail comment-file">
                                        </div>
                                    @endforeach
                                </div>
                            @elseif ($comment->content_type == 'file')
                                <div class="comment-content files">
                                    <a href="{{ route('offers.project-managers.download-file', ['comment' => $comment->id]) }}"
                ...
                @endif
            @elseif ($offer->interests->where('assigned', true)->count() < 1 && $offer->comments->where('user_id',
                    $offer->user_id)->count())
                    @include('layouts.section-header', ['header' => 'Comments', 'icon' => 'icon-line-awesome-comments'])
        ...
        <div class="row mt-5">
            <div class="col-xl-8 col-lg-8 content-right-offset">
                @if ($similar_offers->count())
                    ...
                    <div class="single-page-section">
                        @include('layouts.section-header', ['header' => 'Other Similar Offers', 'icon' =>
        ...
        <div class="sign-in-form">
            ...
            <div class="popup-tabs-container">
                ...
                <div class="popup-tab-content" id="tab">
                    ...
                    <!-- Button -->
                    <button class="button margin-top-35 full-width button-sliding-icon ripple-effect" form="apply-now-form"
...
<script>
    ...
    $(".comment-file").on('click', function(e) {
        ...
        $("#commentFileModal").find('img').attr({
            src: src
        })
