        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <h2>Upgrade your current budget offer</h2>
                    ...
                    <nav id="breadcrumbs" class="dark">
                        <ul>
                            ...
                            <li><a href="{{ route('contests.create') }}">Accept Interest</a></li>
                            ...
                            <div class="alert alert-success">
                                <small>
                                    @if ($user)
                                        Hello {{ $user->username }},
                                    @endif
                                    To accept freelancer interests with prices higher than your budget, please upgrade your budget by paying the difference
                                </small>
                            </div>
            ...
            <div class="col-xl-4 col-lg-4 margin-top-0 margin-bottom-60">
                ...
                <div class="boxed-widget summary margin-top-0">
                    ...
                    <div class="boxed-widget-inner">
                        <ul>
                            <li>Offer Price <span>{{ auth()->user()->is_nigeria == true ? '₦' : '$' }}{{ number_format($amount - $offer->budget) }}</span></li>

                            <li class="total-costs">Final Price <span>{{ auth()->user()->is_nigeria == true ? '₦' : '$' }}{{ number_format($amount - $offer->budget) }}</span></li>
                        </ul>
                    </div>
                </div>
                <!-- Summary / End -->
...
@section('page_scripts')
    ...
    <script>
        ...
        function payWithPaystack() {
            ...
            let budget = parseFloat('{{ getUserCurrencyAmount($user_currency, $offer->budget, $offer->currency, $dollar_rate) }}')
            let interest_amount = parseFloat('{{ getUserCurrencyAmount($user_currency, $amount, $offer->currency, $dollar_rate) }}')
            let amount = interest_amount - budget
        ...
        function saveContestPayment(amount, payment_reference, payment_method) {
            ...
            fetch(`{{ route('offers.project-managers.payment-top-up', ['offer' => $offer->id, 'amount' => $amount]) }}`, {
                    method: 'post',
                    headers: {
                ...
                .then(async responseJson => {
                    if (responseJson.success) {
                        ...
                        setTimeout(() => {
                            window.location = `${webRoot}offers/project-managers/${responseJson.slug}/interested-freelancers`;
                        }, 2000)
                    } else {
                        Snackbar.show({
                            text: responseJson.message,
