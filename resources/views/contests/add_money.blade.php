        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <h2>Add More Money To this contest</h2>

                    <!-- Breadcrumbs -->
                    <nav id="breadcrumbs" class="dark">
                        <ul>
                            <li><a href="{{ route('index') }}">Home</a></li>
                            <li><a href="{{ route('contests.show', $contest->slug) }}">{{ $contest->title }}</a></li>
                            <li>Make Payment</li>
                        </ul>
                    </nav>
                            ...
                            <div class="alert alert-success">
                                <small>
                                    @if ($user)
                                        Hello {{ $user->username }},
                                    @endif
                                    You are about to add {{ $user_currency == 'dollar' ? "$" : '₦' }}{{ number_format($amount) }} to the budget of this contest
                                </small>
                            </div>
            ...
            <div class="col-xl-4 col-lg-4 margin-top-0 margin-bottom-60">
                ...
                <div class="boxed-widget summary margin-top-0">
                    ...
                    <div class="boxed-widget-inner">
                        <ul>
                            <li>Amount To Add <span>{{ auth()->user()->is_nigeria == true ? '₦' : '$' }}{{ number_format($amount) }}</span></li>

                            <li class="total-costs">Final Price <span>{{ auth()->user()->is_nigeria == true ? '₦' : '$' }}{{ number_format($amount) }}</span></li>
                        </ul>
                    </div>
                </div>
                <!-- Summary / End -->
...
@section('page_scripts')
    ...
    <script>
        ...
        function saveContestPayment(amount, payment_reference, payment_method) {
            ...
            fetch(`{{ route('contests.add-money-verification', ['contest' => $contest->id]) }}`, {
                    method: 'post',
                    headers: {
                ...
                .then(async responseJson => {
                    if (responseJson.success) {
                        ...
                        setTimeout(() => {
                            window.location = `{{ route('contests.show', $contest->slug) }}`;
                        }, 2000)
                    } else {
                        Snackbar.show({
                            text: responseJson.message,
