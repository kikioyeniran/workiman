        ...
        function setActiveConversation(active_conversation_user) {
            ...
            conversations.sort((a, b) => b.last_message.created_at < a.last_message.created_at ? -1 : 1).map((
                ...
                conversations_list.append(
                    `<li class="${conversation_user.id == conversation.user_1_id || conversation_user.id == conversation.user_2_id ? 'active-message' : ''}"><a onclick="setConversationAsActive(${index})"><div class="message-avatar"><img src="${webRoot + (conversation.last_message.user.avatar ? `storage/avatars/${conversation.last_message.user.avatar}` : `_home/images/user-avatar-big-02.jpg`)}" alt="" /></div><div class="message-by"><div class="message-by-headline"><h5>${conversation.last_message.user.display_name}</h5><span>${conversation.last_message.created_at_diff}</span></div><p>${conversation.last_message.content}</p></div></a></li>`
                )
            })
